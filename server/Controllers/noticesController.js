...
NoticesController.accountingClaimRequest = async (req, res) => {
    ...
    const cacheKey = 'pending_payments_all';
...
NoticesController.getAllCustomers = async (req, res) => {
    try {
        // Optimizar la consulta con índices y filtros más específicos
        const query = `
            SELECT TOP 1000
                CustomerID,
                Name,
                ...
                Cell,
            WHERE n.paid = 0 
                AND n.CreateDate >= DATEADD(MONTH, -6, GETDATE()) -- Solo últimos 6 meses
                Address,
                AddressNext,
                City,
                ZipCode,
            ...
            ORDER BY AÑO DESC, MES DESC, QUINCENA DESC, ia.Ex_InvoicingAddressID`;
        const request = new sql.Request();
        const result = await request.query(query);
